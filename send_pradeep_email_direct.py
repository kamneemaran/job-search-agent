#!/usr/bin/env python3
"""Send formatted email to Pradeep with SAP jobs - Location, Score, Title, Company, Apply Link"""
...
FROM_EMAIL = "kminterviewer@gmail.com"  # or kamneemaran45@gmail.com
SENDER_NAME = "Job Search Agent"

# SAP jobs for Pradeep
JOBS = [
    {
...
def send_email():
    ...
    if not gmail_app_password:
        ...
        print(f"\nTo send email, set your Gmail App Password:")
        print(f"  export GMAIL_APP_PASSWORD='your-16-char-password'")
        print(f"\nOR provide it when running this script:")
        print(f"  GMAIL_APP_PASSWORD='xxx' python3 send_pradeep_email_direct.py")
        return False
    ...
    try:
        ...
        print(f"   Error: {str(e)}")
        print(f"\n   Check:")
        print(f"   - Gmail App Password is correct (16 characters, no spaces)")
        print(f"   - 2FA is enabled on Gmail account")
        print(f"   - Less secure apps are allowed (or use App Passwords)")
        print(f"\n" + "=" * 80)
        return False
