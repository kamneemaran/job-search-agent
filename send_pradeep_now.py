#!/usr/bin/env python3
"""Send formatted email to Pradeep via /api/digest/send endpoint"""
...
def send_email():
    """Send email via digest/send endpoint"""
    print("=" * 80)
    ...
    try:
        ...
        print(f"\n[2] Sending via {API_BASE}/api/digest/send")
        response = requests.post(
            f"{API_BASE}/api/digest/send",
            json=payload,
            timeout=30
        )
